...
from dataloader_reims_mtf import REIMS_dataset
...
class BaseSSL(nn.Module):
    ...
    DATA_ROOT = '/raid/home/fahimehf/Codes/REIMSVission/Fahimeh_Experiments/Data/'
    ...
    def prepare_data(self):
        ...
        elif self.hparams.data == 'path':
            self.trainset = REIMS_dataset(root=self.DATA_ROOT, transform=train_transform, fold=self.hparams.fold, mode='all_invivo') # all_sup 
            print(self.hparams.fold)
            self.testset = REIMS_dataset(root=self.DATA_ROOT, transform=test_transform, mode='test')
        else:
    ...
    def add_model_hparams(cls, parser):
        parser.add_argument('--data', help='Dataset to use', default='path')
        parser.add_argument('--arch', default='resnet18', help='Encoder architecture')
        ...
        parser.add_argument('--fold', default='0', help='folds')
...
class SimCLR(BaseSSL):
    ...
    def transforms(self):
        ...
        elif self.hparams.data == 'path': #imagenet
...
class SSLEval(BaseSSL):
    ...
    def step(self, batch):
        if self.hparams.problem == 'eval' and self.hparams.data == 'path': #imagenet
            batch[0] = batch[0] / 255.
        h, y = batch
    ...
    def prepare_data(self):
        ...
        def create_emb_dataset(dataset):
            ...
            for x, y in tqdm(loader):
                if self.hparams.data == 'path': #imagenet
